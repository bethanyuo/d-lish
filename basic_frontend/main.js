...
createItem = async () => {
    ...
    if (createItemTypeField.value == null) {
        var nftFile = new Moralis.File("nftFile.jpg",createItemFile.files[0]);
        var nftFileType = 'nftFile.jpg'

    } else {
        var nftFile = new Moralis.File("nftFile.mp4",createItemFile.files[0]);
        var nftFileType = 'nftFile.mp4'
    }
...
renderUserItem = async (item) => {
    ...
    if (item.file == 'nftFile.mp4'){
        userItem.getElementsByTagName("img")[0].src = "images/chef_play_icon.png";
    } else {
    ...
    userItem.getElementsByTagName("button")[0].onclick = async () => {
        ...
        await ensureMarketplaceIsApproved(item.tokenId, item.TOKEN_CONTRACT_ADDRESS);
        await marketplaceContract.methods.addItemToMarket(item.tokenId, item.TOKEN_CONTRACT_ADDRESS, userItem.getElementsByTagName("input")[0].value).send({from: user.get('ethAddress') });
    };
...
renderItem = (item) => {
    ...
    if (item.file == 'nftFile.mp4'){
        itemForSale.getElementsByTagName("img")[1].src = "images/chef_play_icon.png";
    } else {
...
ensureMarketplaceIsApproved = async (tokenId, TOKEN_CONTRACT_ADDRESS) => {
    user = await Moralis.User.current();
    const userAddress = user.get('ethAddress');
...
const createItemTypeField = document.getElementById("selectCreateItemType");
